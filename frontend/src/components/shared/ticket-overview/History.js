...
function History({ ticketComments,ticketId }) {
    ...
    const sendComment = () => {
        ...
        api.post("/ticket-comment/create",{comment: comment,ticketId:ticketId})
            .then((res) => {
    ...
    return (
        ...
        <Snackbar
            ...
            >
                 <Alert onClose={handleClose} severity="error">Niste unijeli odgovor u polje!</Alert>
        </Snackbar>
        <div>
            ...
            <Container style={{ marginLeft: "3%", width: "90%", backgroundColor: "#00101f", padding: 20 }}>
                {commentPage.map(tc =>
                    <Paper key={tc.id} style={{ padding: 15, marginBottom: 20, height: "auto", backgroundColor: "white", fontFamily: "Yantramanav", overflow: "hidden" }}>
                        <span style={{ fontWeight: "bold", float: "right" }}>{dateFormat(tc.dateTime, "dd/mm/yyyy, HH:MM")}</span>
                ...
                {ticketComments.length>0? <Stack spacing={2}>
                ...
                <FormControl required style={{ width: "100%" }} >
                    <Textarea required  placeholder='Unesite odgovor' value={comment ? comment : ""} name="comment" onChange={(event) => { handleCommentChange(event) }} minRows={5} style={{ width: "100%" }} />
                    <Button
                        style={{
